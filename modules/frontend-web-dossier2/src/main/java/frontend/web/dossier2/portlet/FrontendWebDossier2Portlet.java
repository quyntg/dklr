package frontend.web.dossier2.portlet;

import frontend.web.dossier2.constants.FrontendWebDossier2PortletKeys;
...
@Component(
	...
	property = {
		...
		"javax.portlet.name=" + FrontendWebDossier2PortletKeys.FrontendWebDossier2,
		"javax.portlet.resource-bundle=content.Language",
		"javax.portlet.security-role-ref=power-user,user"
	},
	service = Portlet.class
)
public class FrontendWebDossier2Portlet extends FreeMarkerPortlet {
	@Override
	public void render(
...
//				String tmp = listResults.stream().map(Object::toString)
//                        .collect(Collectors.joining(", "));
//				_log.info("tmp+++++++++++++++++++++Join" + tmp);

				writeJSON(actionRequest, actionResponse, listResults.toString());
				// writeJSON(actionRequest, actionResponse, tmp);
...
//				}
		...
		try {
			...
			try {
				...
				for (Document document : listResults) {
					...
					resultCode.add(document.get(DictItemGroupTempTerm.GROUP_CODE).toString());

				}
				JSONObject obj = JSONFactoryUtil.createJSONObject();
				obj.put("groupCode", String.join(",", resultCode));
				writeJSON(resourceRequest, resourceResponse, obj);
				_log.info("data+++++++ccccc+++++++++++++++++++" + obj);

			}
			catch (Exception e) {
				writeJSON(resourceRequest, resourceResponse, resultCode);
				_log.error(e);
			}
	...
	private static final Log _log = LogFactoryUtil.getLog(FrontendWebDossier2Portlet.class);
}