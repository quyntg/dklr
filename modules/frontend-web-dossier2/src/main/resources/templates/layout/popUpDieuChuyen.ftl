<template>
  <v-layout wrap>
    ...
    <v-flex xs12 sm12 style="border: 1px solid #ccc; position: relative;" class="mb-2">
      ...
      <v-layout row wrap>
        ...
        <v-flex xs12 sm2 class="text-xs-right py-0">
          <label class="pt-2">Tổ tổng hợp, đơn vị khai thác</label>
        </v-flex>
        <v-flex xs12 sm4 class="py-0">
          <v-select
          :items="doiPhongItems"
          ...
          :disabled="!stateEditDc"
          autocomplete
          v-model="thongTinPhieu['purchaserCorporationId']"
          class="pt-0"
          ></v-select>
        </v-flex>
        <v-flex xs12 sm2 class="text-xs-right py-0">
  ...
  <template slot="items" slot-scope="props">
    ...
    <td class="text-xs-center pt-2">{{ props.index + 1 }}</td>
...
<v-flex xs12 sm12>
  <v-btn color="primary" small @click="addNhapKho()" :disabled="!stateEditDc"><v-icon>add</v-icon> Thêm dòng</v-btn>
  <v-btn color="primary" small @click="deleteVatTuSelect()" :disabled="!stateEditDc" v-if="selected && selected['length']"><v-icon>delete</v-icon> Xóa dòng</v-btn>
  <v-btn color="primary" small @click="luuPhieu(0, '')" :disabled="!stateEditDc"><v-icon>save</v-icon> Lưu phiếu</v-btn>
  <v-btn color="primary" small @click="luuPhieu(1, '')" :disabled="!stateEditDc"><v-icon>done</v-icon> Xác nhận</v-btn>
