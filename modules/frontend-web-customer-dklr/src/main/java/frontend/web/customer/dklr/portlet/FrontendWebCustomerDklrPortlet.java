...
public class FrontendWebCustomerDklrPortlet extends FreeMarkerPortlet {
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		try {
			...
			_log.info("dossierObj=======================" + dossierObj);
		} catch (Exception e) {
...
//			JSONObject registrationObj = JSONFactoryUtil.createJSONObject(registrationStr);
			...
			if (jApplicantProfile != null && Validator.isNotNull(jApplicantProfile)) {
				renderRequest.setAttribute("registration", jApplicantProfile);
				_log.info("registration===========================" + jApplicantProfile);
			}
		} catch (Exception e) {
		...
		String dossierNo = ParamUtil.getString(renderRequest, "dossierNo");
		String methodOfIssue = ParamUtil.getString(renderRequest, "methodOfIssue");
		String dnGiamSat = ParamUtil.getString(renderRequest, "dnGiamSat");
		String dnKiemTra = ParamUtil.getString(renderRequest, "dnKiemTra");
		String dnHuy = ParamUtil.getString(renderRequest, "dnHuy");
		String dnTraHuy = ParamUtil.getString(renderRequest, "dnTraHuy");
		String issueType = ParamUtil.getString(renderRequest, "issueType");
		...
		renderRequest.setAttribute("dossierNo", dossierNo);
		renderRequest.setAttribute("methodOfIssue", methodOfIssue);
		renderRequest.setAttribute("dnGiamSat", dnGiamSat);
		renderRequest.setAttribute("dnKiemTra", dnKiemTra);
		renderRequest.setAttribute("dnHuy", dnHuy);
		renderRequest.setAttribute("dnTraHuy", dnTraHuy);
		renderRequest.setAttribute("issueType", issueType);
	...
	private JSONObject createConstants() {
		...
		dossierNew.put("text", "Hồ sơ chưa gửi");
		...
		dossierDone.put("text", "Hồ sơ đã hoàn thiện");
		...
		JSONObject dossierChoKhacPhuc = JSONFactoryUtil.createJSONObject();
		dossierChoKhacPhuc.put("text", "Hồ sơ chờ khắc phục");
		dossierChoKhacPhuc.put("value", "waiting_4");
		dossierChoKhacPhuc.put("valueSub", "waiting_4");
		dossierStatus.add(dossierChoKhacPhuc);
		
		JSONObject dossierDaKhacPhuc = JSONFactoryUtil.createJSONObject();
		dossierDaKhacPhuc.put("text", "Hồ sơ đã khắc phục");
		dossierDaKhacPhuc.put("value", "waiting_5");
		dossierDaKhacPhuc.put("valueSub", "waiting_5");
		dossierStatus.add(dossierDaKhacPhuc);
		...
		dossierExpired.put("text", "GCN đến hạn XN hiệu lực");
	...
	private String getLabelApplicantNote(String resCancelling, String sendAdd, String sendReissue) {
		...
		if (sendReissue != null && sendReissue != "") {
			return "Thông báo yêu cầu sửa giấy chứng nhận";
		}
	...
	private JSONObject generateURLJsonObject(RenderResponse renderResponse) throws WindowStateException {
		...
		PortletURL danhSachAnChiDangKiem = renderResponse.createRenderURL();
		danhSachAnChiDangKiem.setWindowState(LiferayWindowState.EXCLUSIVE);
		danhSachAnChiDangKiem.setParameter("mvcPath", "/templates/danh_sach_an_chi_dang_kiem.ftl");

		urlObject.put("danh_sach_an_chi_dang_kiem", danhSachAnChiDangKiem);
		...
		PortletURL danhSachXeChoInPhieuXuatXuongChiTiet = renderResponse.createRenderURL();
		danhSachXeChoInPhieuXuatXuongChiTiet.setWindowState(LiferayWindowState.EXCLUSIVE);
		danhSachXeChoInPhieuXuatXuongChiTiet.setParameter("mvcPath", "/templates/danh_sach_xe_cho_in_phieu_xuat_xuong_chi_tiet.ftl");

		urlObject.put("danh_sach_xe_cho_in_phieu_xuat_xuong_chi_tiet", danhSachXeChoInPhieuXuatXuongChiTiet);
