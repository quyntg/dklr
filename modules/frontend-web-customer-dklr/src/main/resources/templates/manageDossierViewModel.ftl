...
<script type="text/javascript">
	...
	var showAlert = function (title, message, okText){
		$("#confirm").html("");
		var cf = $("#confirm").kendoDialog({
			width: "400px",
			title: title,
	...
	var fnLoadStatus = function (id, start, end) {
	...
	var fnGetCounter = function () {
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatus li[dataPk='waiting_4'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatus li[dataPk='waiting_5'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			url : "/o/rest/v2/dossiers/expiredcertificates/count/EXPIRED",
			dataType : "json",
			type : "GET",
			data : {
				"dossierStatus": "processing",
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatus li[dataPk='DNXNHL'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatus li[dataPk='XNHL'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatus li[dataPk='TBDSX'] .bagde").html(result.total);
			},
			error : function(result){
		...
		function () {
			if ($("#profileStatusAnChi li[dataPk='traCuuPTXX'] .bagde").html() != '0') {
				return $("#profileStatusAnChi li[dataPk='traCuuPTXX'] .bagde").html();
			}
			return $.ajax({
				url : "/o/rest/vr-app/count/getDataGCN",
				dataType : "json",
				type : "GET",
				data : {
					"applicantIdNo": "${(applicant.applicantIdNo)!}"
		...
		$.ajax({
			url : "/o/rest/vr-app/outputsheetdetail/count/data",
			dataType : "json",
			type : "GET",
			data : {
				"applicantIdNo": "${(applicant.applicantIdNo)!}"
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChi li[dataPk='anChiDaCapPhat'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			url : "/o/rest/vr-app/issue/count/data",
			dataType : "json",
			type : "GET",
			data : {
				"applicantIdNo": "${(applicant.applicantIdNo)!}"
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChi li[dataPk='choinPhieuXX'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChiDk li[pk='hsChuaQuyetToan'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChiDk li[pk='hsDaQuyetToan'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChiDk li[pk='hsCapTheoLo'] .bagde").html(result.total);
			},
			error : function(result){
		...
		$.ajax({
			...
			headers : {"groupId": "${groupId}"},
			success : function(result){
				$("#profileStatusAnChiDk li[pk='hsDaGiamSat'] .bagde").html(result.total);
			},
			error : function(result){
	...
	var dataSourceProfile = new kendo.data.DataSource({
		transport: {
			read: function (options) {
				...
				if (options['data'] && (options['data']['id'] === "HSDHXNHL")) {
					var filter = options['data']['stateFilter'] ? options['data']['stateFilter'] : 'EXPIRED'
					if ($(".displayCheckBox input").is(":checked")) {
						filter = 'TERM';
					} else {
						filter = 'EXPIRED';
					}
					url = "/o/rest/v2/dossiers/expiredcertificates/" + filter
					options['data']['applicantIdNo'] = "${(applicant.applicantIdNo)!}";
					if (filter == 'TERM') {
						options['data']['expiredStatus'] = '0,1'
					}
					options['data']['start'] = (options['data']['start'] || options['data']['start'] == 0) ? options['data']['start'] : -1
					options['data']['end'] = (options['data']['end'] || options['data']['end'] == 0) ? options['data']['end'] : -1
					delete options['data']['stateFilter']
				} else if (options['data'] && options['data']['id'] === "DNXNHL" || options['data']['id'] === "XNHL") {
					var filter = options['data']['stateFilter'] ? options['data']['stateFilter'] : 'EXPIRED'
					url = "/o/rest/v2/dossiers/expiredcertificates/" + filter
					options['data']['applicantIdNo'] = "${(applicant.applicantIdNo)!}";
					if (filter == 'TERM') {
					...
					options['data']['end'] = (options['data']['end'] || options['data']['end'] == 0) ? options['data']['end'] : -1
					delete options['data']['stateFilter']
				} else if (options['data'] && options['data']['id'] === "TBDSX") {
					url = "/o/rest/v2/dossiers/expiredcertificates/EXPIRED"
					options['data']['applicantIdNo'] = "${(applicant.applicantIdNo)!}";
					options['data']['start'] = (options['data']['start'] || options['data']['start'] == 0) ? options['data']['start'] : -1
					options['data']['end'] = (options['data']['end'] || options['data']['end'] == 0) ? options['data']['end'] : -1
					delete options['data']['stateFilter']
				} else {
					if(!options['data']['state'] && !options['data']['status']) {
						options.success({
							data: dataSourceProfile.data() || [],
							total: dataSourceProfile.data()['length'] || 0
						});
						return;
					}
				}
				...
				$.ajax({
					...
					data: options['data'],
					success: function(result) {
						if (result['total'] && result['total'] != 0 && result['data']) {
							var indexItem = result['total'] + 1;
							for (var i = 0; i < result.data['length']; i++) {
								indexItem -= 1;
								result.data[i]['count'] = indexItem;
								if (options['data'] && (options['data']['id'] === "HSDHXNHL")) {
									result.data[i]['isExpiredCertificates'] = true
									result.data[i]['isHSDHXNHL'] = true;
								} else {
									result.data[i]['isExpiredCertificates'] = false
									result.data[i]['isHSDHXNHL'] = false;
								}
								if (options['data'] && (options['data']['id'] === "DNXNHL")) {
									result.data[i]['isDNXNHL'] = true
								} else {
									result.data[i]['isDNXNHL'] = false
								}

								if (options['data'] && (options['data']['id'] === "TBDSX")) {
									result.data[i]['isTBDSX'] = true;
								} else {
									result.data[i]['isTBDSX'] = false;
								}
								if (!result.data[i]['correctingDate']) {
									result.data[i]['correctingDate'] = ''
								}
								if (!result.data[i]['statusReg']) {
									result.data[i]['statusReg'] = ''
								}
								if (!result.data[i]['endorsementDate']) {
									result.data[i]['endorsementDate'] = ''
								}
								if (!result.data[i]['cancellingDate']) {
									result.data[i]['cancellingDate'] = ''
								}
								if (result.data[i]['submitDate']) {
									result.data[i]['submitDate'] = formatDateFn(result.data[i]['submitDate']);
								}
								if (result.data[i]['receiveDate']) {
									result.data[i]['receiveDate'] = formatDateFn(result.data[i]['receiveDate']);
								}
								// if (result.data[i]['certDate']) {
								// 	result.data[i]['certDate'] = formatDateFn(result.data[i]['certDate']);
								// }
							}
	...
	var modelPanel = kendo.observable({
		...
		filterDossierNo: function(e){
			...
			} else if(statusDossier == "endorsement"){
				dataSourceProfile.read({
					"dossierNo" : $("#dossier-emp-nav-selectbox-by-dossierNo").val(),
					"service": $("#serviceInfo").val(),
					"keyword": $("#keyInput").val(),
			...
			} else if(statusDossier == "correcting"){
				dataSourceProfile.read({
					"dossierNo" : $("#dossier-emp-nav-selectbox-by-dossierNo").val(),
					"service": $("#serviceInfo").val(),
					"keyword": $("#keyInput").val(),
		...
		filterStatus: function(e){
			...
			dossierIdDnQuyetToan = 0;
			window.sessionStorage.setItem('dossierIdBanKhaiHoSoXNHL', dossierIdBanKhaiHoSoXNHL);
			window.sessionStorage.setItem('dossierIdDnQuyetToan', dossierIdDnQuyetToan);
			window.sessionStorage.setItem('dossierIdCTNHoSoXNHL', '');
			window.sessionStorage.setItem('briefNoteHoSoXNHL', '');
		...
		load_serviceConfig:function(e){
			...
			window.sessionStorage.setItem("dossierIdDnQuyetToan", '');
		},
		dataBound: function() {
		...
		filterInvestigation: function(e){
			e.preventDefault();
			$("#profileStatusAnChi li").css('background', '#F6F6F6');
			$(e.currentTarget).css('background', 'linear-gradient(to right, rgb(255, 234, 160) 0%, rgba(246, 246, 246, 0) 100%)');
			firstLoadDataSource = false;
			manageDossier.navigate("/tra-cuu/tra-cuu-ho-so");

			fnLoadStatus("all", 0, 10);
			$("#statusName").html('Tra cứu hồ sơ');
			firstLoadDataSource = true;
		},
		filterDanhSachAnChiDk: function(e) {
			e.preventDefault();
			$("#profileStatusAnChiDk li").css('background', '#F6F6F6');
			$(e.currentTarget).css('background', 'linear-gradient(to right, rgb(255, 234, 160) 0%, rgba(246, 246, 246, 0) 100%)');
			// $("#statusName").html($(".itemStatus.active .dossierStatus").text());
			var route = $(e.currentTarget).attr('datapk') || '';
			console.log('route========', route, $(e.currentTarget))
			manageDossier.navigate(route)
		},
	...
	var createDossierXNHL = function(dossierTemplateNo, serviceCode, govAgencyCode, dossierId){
	...
	var guiHoSoKhacPhuc = function () {
		$(".guiHoSoKhacPhuc").click( function (e) {
			e.stopPropagation();
			var id = $(this).attr("data-Pk");
			manageDossier.navigate("/waiting_4/dossiers/" + id);
		});
	}
	...
	var deNghiXNHH = function () {
		$(".deNghiXNHH").click( function (e) {
			...
			window.sessionStorage.setItem('expireCertificateId', tmp['expireCertificateId'] || tmp['vrExpireCertificateId']);
			console.log('tmp=========', tmp)
			...
			window.sessionStorage.setItem('dossierIdCTNHoSoXNHL', tmp['dossierIdCTN']);
			window.sessionStorage.setItem('briefNoteHoSoXNHL', tmp['briefNote']);
			console.log('tmp===params======', dossierTemplateNo, serviceCode, govAgencyCode)
			if (dossierTemplateNo && serviceCode && govAgencyCode) {
				createDossierXNHL(dossierTemplateNo, serviceCode, govAgencyCode, tmp['dossierId'])
			}
		});
	};
	var dungSX = function () {
		$(".dungSX").click( function (e) {
			e.stopPropagation();
			var id = $(this).attr("data-Pk");
			var dossierIdCTN = $(this).attr("data-dossierIdCTN");
			dossierIdBanKhaiHoSoXNHL = 0;
			dossierIdDnQuyetToan = 0;
			window.sessionStorage.setItem('dossierIdBanKhaiHoSoXNHL', '');
			window.sessionStorage.setItem('dossierIdDnQuyetToan', '');
			window.sessionStorage.setItem('dossierIdCTNHoSoXNHL', '');
			window.sessionStorage.setItem('briefNoteHoSoXNHL', '');
			...
			if (cf) {
				$.ajax({
					url: "/o/rest/vr-app/vrvehicletypecertificate/vrexpirecertificate/update/DSX/" + dossierIdCTN,
					dataType: "json",
					type: "PUT",
					data: {
						expiredStatus: 3
	...
	var modelMain = kendo.observable({
		...
		onChangeSwitch: function (e) {
			...
			if (isCheck) {
				dataSourceProfile.read({
					"dossierNo" : $("#dossier-emp-nav-selectbox-by-dossierNo").val(),
					"serviceCode": $("#serviceInfo").val() || "",
					"applicantIdNo": "${(registration.applicantCode)!}",
					"keyword": $("#keyInput").val(),
					"stateFilter": "TERM",
					"id": statusDossier
				})
				$("#labelSwitch").text("Danh sách GCN còn hiệu lực");
			} else {
				dataSourceProfile.read({
					"dossierNo" : $("#dossier-emp-nav-selectbox-by-dossierNo").val(),
					"serviceCode": $("#serviceInfo").val() || "",
				...
				$("#labelSwitch").text("Thông báo từ cơ quan quản lý");
			}
		},
