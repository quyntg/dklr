...
<div class="accordion-group">
	<div class="accordion-heading">
		<a class="accordion-toggle" data-toggle="collapse" href="#group7">
			BẢO HÀNH BẢO DƯỠNG
		</a>
	</div>
	<div id="group7" class="accordion-body collapse toggle-hide" style="background-color: #ffffff">
		<div class="accordion-inner">
			<#-- <div class="PT5">
				<div class="ML40 col-sm-6 P5 PL0">
					<span> 
...
<script type="text/javascript">
	...
	function checkCSBHBD () {
		$("#csbhbdId").hide()
		$.ajax({
			url : "/o/rest/vr-app/applicant/profile/search?mtcore=1&markupmaintainer=4&applicantlock=0&start=-1&end=-1&applicantCode=0106752923",
			dataType : "json",
			type : "GET",
			data : {
				mtcore: 1,
	...
	var createDossier = function(dossierTemplateNo, serviceCode, govAgencyCode, selector){
		...
		if ("${(registration.id)!}" && "${(registration.applicantStatus)!}" == 2) {
			...
			$.ajax({
				...
				data : {
					...
					contactEmail : "${(registration.applicantEmail)!}"
				},
				headers : {"groupId": ${groupId}},
				success : function(result){
			...
			notification.show({
				message: "Tài khoản chưa bổ sung thông tin doanh nghiệp, vui lòng bổ sung thông tin!"
			}, "error");
		}
