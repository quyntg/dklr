				...
				'events': {
					...
					getTrackChanges: function (callback) {
						...
						$.ajax({
							...
							success: function (response) {
								let data = response['nextContentFileEntryId'] ? JSON.parse(response['nextContentFileEntryId']) : ''
								if (data) {
									let tmp = {
					...
					generalHtml2: function (mang_ts) {
						...
						$('#column-auto').html('');
						var var_array = []
						var html_code = '', html_code1 = '', html_code2 = '', html_code3 = '';
						...
						var index_var = 0, check_loai_dong_co = 0, html_loai_dong_co = '';
					...
					toDetailSoatXetGCN(item, index) {
						...
						if (item['chi_tiet']) {
							...
							$('#dsXePopUpCapPhieu table.jexcel.jexcel_overflow > colgroup>col').each((index, item) => {
					...
					closeAndSaveCop: function (argument) {
						...
						let processActionId = $("#formCOP").attr('processActionId') || ''
					...
					closeAndSaveCSBHBD: function (argument) {
						...
						let referenceUid = $("#formBaoCaoCSBHBD").attr('referenceUid') || ''
						let processActionId = $("#formBaoCaoCSBHBD").attr('processActionId') || ''
						vm.submitAlpacajsForm({
							eform: true,
							dossierPartId: 161,
							partNo: 'KQ2',
					...
					xacNhanBBKiemTraGSKT() {
						...
						if (vm.bienBanGSModel && vm.bienBanGSModel['chi_tiet_2']) {
							let sld = 0
							vm.bienBanGSModel['chi_tiet_2'].forEach(item => {
					...
					toDetailBienBanKTGS_DotXuat(item, index) {
						let vm = this
					...
					setPointFormCSBHBD(item, index, key) {
						let vm = this
					...
					xemGiayChungNhan(item) {
						...
						let url = '/o/rest/v2/dossiers/fileEntryId/' + item['vr_VehicleTypeCertificate']['deliverableFileEntryid']
						let configs = {
							headers: {
							},
							responseType: 'blob'
						...
						var promise = new Promise(function (resolve, reject) {
							...
							axios.post(url, queries).then(function (response) {
								...
								if (typeof serializable['hits']['total'] === 'number') {
									...
									let totalTmp = serializable['hits']['total'] ? serializable['hits']['total']['value'] : 0
					...
					tickAll_DX_LD: function (val) {
						...
						if (vm.bienBanDXModel['chi_tiet_2'] && vm.bienBanDXModel['chi_tiet_2']['length']) {
							vm.bienBanDXModel['chi_tiet_2'] = vm.bienBanDXModel['chi_tiet_2'].map(item => {
								item['kq_ld'] = val
								return item
							})
						}
					...
					tickAllKL: function (val) {
						...
						if (vm.bienBanGSModel['chi_tiet_2'] && vm.bienBanGSModel['chi_tiet_2']['length']) {
							vm.bienBanGSModel['chi_tiet_2'] = vm.bienBanGSModel['chi_tiet_2'].map(item => {
								item['kl'] = val
								return item
							})
						}
					...
					deleteBienBanKTGS: function () {
						...
						vm.$dialog.confirm('Bạn có chắc chắn muốn xóa!', {
							...
							dialog.close();
							vm.selected.forEach((itemSelect) => {
								let index = vm.dsBBKiemTraGS.findIndex(item => {
					...
					selectSoGCNCapPhieu(item, index) {
						...
						if (item) {
							...
							$('#dsXeCapPhieuHoSo table.jexcel.jexcel_overflow > colgroup>col').each((index, item) => {
								console.log('item====', $(item).attr('width'))
								$("tbody.draggable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
								$("thead.resizable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
							});
					...
					chiTietPhieu: function (item) {
						...
						$('#chiTietPhieuTable table.jexcel.jexcel_overflow > colgroup>col').each((index, item) => {
							console.log('item====', $(item).attr('width'))
							$("tbody.draggable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
							$("thead.resizable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
						});
						...
						$('#dsSoSerial table.jexcel.jexcel_overflow > colgroup>col').each((index, item) => {
							console.log('item====', $(item).attr('width'))
							$("tbody.draggable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
							$("thead.resizable>tr>td:nth-child(" + (index + 1) + ")").css('min-width', $(item).attr('width') + 'px')
						});
						...
						for (let i = 0; i < vm.part1CSBHBD.length; i++) {
							if (vm.part1CSBHBD[i].hasOwnProperty('nd_result') && vm.part1CSBHBD[i]['nd_result']) {
								...
								if (vm.detailCSBHBD[vm.part1CSBHBD[i]['nd_result']] === 2 || vm.detailCSBHBD[vm.part1CSBHBD[i]['nd_result']] === 3 || vm.detailCSBHBD[vm.part1CSBHBD[i]['nd_result']] === 4) {
									let obj = Object.assign({}, vm.part1CSBHBD[i])
					...
					convertCSBHBDReportRepository() {
						let vm = this
						let markupDesigner = 0
						let markupOverseasManufacturer = 0
						let markupDomesticsManufacturer = 1
						let markupMaintainer = 0
						let markupImporter = 0
					...
					convertCSBHBDProductionPlantEquipment() {
						let vm = this
						let vr_COPProductionPlantEquipment = vm.detailCSBHBD['thiet_bi_xuat_xuong'] || []
						if (vr_COPProductionPlantEquipment && vr_COPProductionPlantEquipment['length']) {
							vr_COPProductionPlantEquipment = vr_COPProductionPlantEquipment.map(item => {
								let MarkupXCG = 1
								let MarkupXCGNK = 0
					...
					initFormPopupBienBanThietBiCSBHBD() {
						let vm = this
						if (vm.detailCSBHBD['thiet_bi_xuat_xuong'] && vm.detailCSBHBD['thiet_bi_xuat_xuong']['length']) {
							...
							for (let i = 0; i < vm.detailCSBHBD['thiet_bi_xuat_xuong'].length; i++) {
					...
					showAlpacaJSFORM: function (item, dossierActionId) {
						...
						if (item.eform && item['dossierPartId'] === 288 && item['partNo'] === 'KQ4') {
							...
							// set default đạt vào ô tick đạt ko đạt
							for (let f = 0; f < vm.part1CSBHBD.length; f++) {
					...
					// getNameDKV () {
					// 	let vm = this
					// 	let userId = vm.detailCSBHBD['dang_kiem_vien_1']
					// 	let itemDkv = vm.dangKiemVienItems.find(item => {
					...
					postNextActions: function (item) {
						...
						if (item['postStepCode'] === 139) {
							let arrData = []
							if (vm.detailSoatXetGCN['bang_khai'] && vm.detailSoatXetGCN['bang_khai']['length']) {
								vm.detailSoatXetGCN['bang_khai'].forEach(item => {
							...
							let url = "/o/rest/vr-app/issue/inspectionrecord/" + vm.detailModel['dossierId']
				...
				'events': {
					_inilistDocumentIn: function (item) {
						...
						axios.all([axios.get(url, config), axios.get(urlFiles, config)]).then(axios.spread(function (urlRespones, urlFilesRespones) {
							...
							vm.listDocumentOutItems.forEach(item => {
